<form method="POST" action="{{ route('login') }}" class="space-y-4">
    ...
    <div class="fromGroup">
        ...
        <div class="relative ">
            <input type="email" name="email" id="email"
                ...
                placeholder="{{ __('Escriba su email') }}" autofocus
                value="{{ old('email') }}">
    ...
    {{-- Password --}}
    <div class="fromGroup">
        <label for="password" class="block capitalize form-label">{{ __('Contraseña') }}</label>
        <div class="relative ">
    ...
    <div class="flex justify-between">
        <div class="checkbox-area">
            <label class="inline-flex items-center cursor-pointer" for="remember_me">
                ...
                <span class="text-slate-500 dark:text-slate-400 text-sm leading-6">{{ __('Recordarme') }}</span>
            </label>
        </div>
